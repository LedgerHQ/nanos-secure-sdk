...
for node in calls:
    ...
    if not func_name in defined_calls:
      ...
      if params[0][0] != "void":
          s+="  unsigned int parameters ["+str(OFFSET_FIRST_PARAM_ALLOC)+"+"+str(len(params))+"];\n"
          ...
          s+="  unsigned int parameters ["+str(OFFSET_FIRST_PARAM_ALLOC)+"];\n"

      if 1:
        s+="  parameters[0] = (unsigned int)\""+libname+"\";\n"
        s+="  parameters[1] = (unsigned int)"+const_id_in+";\n"
      ...
      if params[0][0] != "void":
          ...
          for param in params:
              s+="  parameters["+str(i)+"] = (unsigned int)"+str(param[1])+";\n"
              callargs += param[1]
              j+=1
              if (j != len(params)):
                callargs += ", "
      ...
      dispatcher_case = "          case "+const_id_in+":\n"

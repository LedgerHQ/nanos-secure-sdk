...
SYSCALL unsigned int os_endorsement_get_public_key(unsigned char index, unsigned char* buffer PLENGTH(65), unsigned char *length PLENGTH(1));
SYSCALL unsigned int os_endorsement_get_public_key_certificate(unsigned char index, unsigned char* buffer PLENGTH(ENDORSEMENT_MAX_ASN1_LENGTH), unsigned char* length PLENGTH(1));

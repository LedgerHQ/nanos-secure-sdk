...
#define GET_SOUND_SETTING_VALUE(notif_setting, feedback_setting) \
    ((uint32_t) ((!(feedback_setting & 0b01)) | ((!(notif_setting) << 1) & 0b10)))
...
/**
 * Define a setting's value from a user buffer and its length. In case of error, a throw is
 * executed.
 */

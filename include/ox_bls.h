 ...
 *
 * @param[in] secret         Pointer to either random bytes used to derive the private key or the private key itself.
 *
 ...
 *
 * @param[in] public_key     Pointer to the public key. This is NULL for 'basic' and 'proof of possession' schemes.
 *
 ...
 *                      - CX_MEMORY_FULL
 */
SYSCALL cx_err_t cx_hash_to_field(const uint8_t *msg, size_t msg_len,
 ...
 *                           The fist bit is always 1 to indicate that the signature is in compressed form.
 *                           The second bit is set iff the result point is the point at infinity, i.e. all

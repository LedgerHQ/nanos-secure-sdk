...
#define SEPROXYHAL_TAG_SESSION_START_EVENT_BOOTMENU  0x08
...
#define SEPROXYHAL_TAG_BLE_SECURITY_DB_LOADED_EVENT \
    0x00  // the security db chunk has ben loaded into MCU's RAM, the SE can proceed with the next
          // one
...
#define SEPROXYHAL_TAG_MCU_TYPE_BD_ADDR 0x03
...
#define SEPROXYHAL_TAG_UNSEC_CHUNK_READ_EXT \
    0x33  // <length:U2BE> <continue=0|restart(bit1)|specifyoffset(bit2)> <offset:U4BE>
#define SEPROXYHAL_TAG_BLE_SEND 0x38  // <opcode(U2BE)> <hci packet payload>
#define SEPROXYHAL_TAG_SET_SCREEN_CONFIG \
    0x3E  // <flags(1byte):pwron(128)|rotation(0:0,90:2,180:4,270:6)|invert(1)> <brightness
          // percentage(1byte)>
...
// TODO use a pairing key between the SE and MCU to decrypt/encrypt the content of the pairing DB.
#define SEPROXYHAL_TAG_BLE_SECURITY_DB_CMD_READ \
    0x01  // -noarg- request the content of the security db to be transmitted through
          // SEPROXYHAL_TAG_BLE_SECURITY_DB_EVENT
#define SEPROXYHAL_TAG_BLE_SECURITY_DB_CMD_WRITE \
    0x02  // <off2BE> <data> request to write a chunk of the security db at the given offset
...
#define SEPROXYHAL_TAG_USB_EP_PREPARE \
    0x50  // <epnum> <direction:setup/in/out/stall/un-stall> <expected_length> <data>
...
#define SEPROXYHAL_TAG_REQUEST_STATUS \
    0x52  // no args, request power levels of all peripherals and current charging state or not if a
          // battery is present.
#define SEPROXYHAL_TAG_RAPDU 0x53  // raw response apdu transport
#define SEPROXYHAL_TAG_I2C_XFER \
    0x54  // <flags:b0=Start,b1=Stop,b2=Moreexpected> <address+R/w> <write:rawdata,read=length(1B)>
...
// #define SEPROXYHAL_TAG_GENERAL_STATUS_MORE_COMMAND 0x0001 // it's a status, but it shall be a
// command instead to avoid perturbating the simple seproxyhal bus logic #define
// SEPROXYHAL_TAG_GENERAL_STATUS_ERROR        0x0002 // shall be a command instead
...
#define SEPROXYHAL_TAG_SCREEN_ANIMATION_STATUS \
    0x68  // <kind(1byte)> <parameters(variable)> // replied with a display processed event when
          // done
#define SEPROXYHAL_TAG_SCREEN_ANIMATION_STATUS_VERTICAL_SPLIT_SLIDE \
    0x00  // param[0:1](BE) = split Y coordinate, param[2:3](BE) = animation duration in ms

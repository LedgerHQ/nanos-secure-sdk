...
__attribute__((always_inline)) inline void
os_perso_derive_eip2333_inline(cx_curve_t curve, unsigned int *path,
  ...
  __asm volatile("mov r0, %1\n"
                 "mov r1, %0\n"
                 "svc #1" ::"r"(parameters),
                 "r"(SYSCALL_os_perso_derive_eip2333_ID_IN)
                 : "r0", "r1");
  if (((volatile unsigned int *)parameters)[0] !=
      SYSCALL_os_perso_derive_eip2333_ID_OUT) { // force reread, updated by the
                                                // svc
    THROW(EXCEPTION_SECURITY);
  }
}
...
os_bootloader_version_inline(unsigned char *version, unsigned int maxlength) {
  ...
  __asm volatile("mov r0, %1\n"
                 ...
                 : "r0", "r1");
  if (((volatile unsigned int *)parameters)[0] !=
      SYSCALL_os_bootloader_version_ID_OUT) { // force reread, updated by the
                                              // svc
    THROW(EXCEPTION_SECURITY);
  }
  return (unsigned int)(((volatile unsigned int *)parameters)[1]);

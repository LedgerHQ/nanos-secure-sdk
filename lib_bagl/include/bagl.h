...
typedef enum bagl_components_type_e_ {
    ...
    BAGL_ICON,
    BAGL_CIRCLE,
    BAGL_LABELINE,  // label for which y coordinate is the baseline of the text, does not support
                    // vertical alignment
#if defined(HAVE_INDEXED_STRINGS)
    BAGL_LABELINE_LOC,  // Used to store index of string instead of a char *text.
...
typedef struct {
    ...
    // unsigned char const * bitmap; // save space by only keeping the offset (2 bytes) instead of
    // the char address (4 bytes)
...
#if defined(HAVE_LANGUAGE_PACK)
#define PIC_LANGU(x) ((const LANGUAGE_PACK *) (PIC(x)))
#endif  // defined(HAVE_LANGUAGE_PACK)
...
typedef struct bagl_animated_s {
    ...
    // NOTE: stroke holds the pause time (unit is 100ms) when reaching each ends of the content to
    // scroll.
    bagl_component_t c;

        ...
        if (ch < font->first_char || ch > font->last_char) {
            ...
            if (ch >= 0xC0) {
                ...
                if (font_symbols != NULL) {
                    ch_bitmap = &PIC_BMP(
                        font_symbols
                            ->bitmap)[PIC_CHAR(font_symbols->characters)[ch & 0x1F].bitmap_offset];
                    ch_width  = PIC_CHAR(font_symbols->characters)[ch & 0x1F].char_width;
        ...
        */
        ...
        if (ch_bitmap) {
            bagl_hal_draw_bitmap_within_rect(
                ...
                ch_bitmap,
                font->bpp * ch_width
                    * ch_height);  // note, last parameter is computable could be avoided
        }
        else {
            ...
            if (context && context_length) {
                ...
                if (strwidth > component->width && context_length > 4) {
                    ...
                    while (strwidth > component->width && ellipsis_1_len
                           && (context_length
                               - ((unsigned int) ellipsis_2_start - (unsigned int) context))) {
...
void bagl_animate(bagl_animated_t *anim, unsigned int timestamp_ms, unsigned int interval_ms)
{
    ...
    if ((anim->next_ms != 0 && anim->next_ms > timestamp_ms) || anim->c.width == 0
        || anim->c.icon_id == 0 || (anim->current_x & 0xF0000000) == 0x40000000) {
        return;
    }
    ...
    if (anim->text && anim->text_length && (type == BAGL_LABELINE || type == BAGL_LABEL)) {
        ...
        unsigned int maxcharwidth
            = bagl_compute_line_width(anim->c.font_id, 0, "W", 1, anim->text_encoding) + 1;
        baseline = font->baseline_height;
        // char_height = font->char_height;

        totalwidth = bagl_compute_line_width(
            anim->c.font_id, 0, anim->text, anim->text_length, anim->text_encoding);
        ...
        remwidth  = bagl_compute_line_width(anim->c.font_id,
                                           0,
                                           anim->text + anim->current_char_idx,
                                           anim->text_length - anim->current_char_idx,
                                           anim->text_encoding);
        charwidth = bagl_compute_line_width(
            anim->c.font_id, 0, anim->text + anim->current_char_idx, 1, anim->text_encoding);
        ...
        // consider the current char of the string has been displayed on the screen (or at least a
        // part of it) viewport         |    < width >   | totalwidth  | a text that does not fit
        // the viewport | rem width        |xt that does not fit the viewport |           s
        //
        unsigned int current_char_displayed_width
            = (anim->current_x & ~(0xF0000000)) - (totalwidth - remwidth);

        // draw a bg rectangle on the area before painting the animated value, to clearup glitches
        // on both sides
        ...
        // crop the viewport
        bagl_hal_draw_rect(anim->c.bgcolor, anim->c.x - maxcharwidth, a, maxcharwidth, b);
        bagl_hal_draw_rect(anim->c.bgcolor, anim->c.x + anim->c.width, a, maxcharwidth, b);
        ...
        switch (anim->current_x & 0xF0000000) {
            ...
            case 0:
                ...
                if (current_char_displayed_width >= charwidth) {
                    ...
                    if (remwidth - current_char_displayed_width <= anim->c.width) {
                        anim->current_x
                            = (totalwidth - remwidth + current_char_displayed_width) | 0x10000000;
                        break;
                    }
                }
            ...
            case 0x10000000:
                ...
                anim->current_x
                    = (totalwidth - remwidth + current_char_displayed_width) | 0x20000000;
                break;

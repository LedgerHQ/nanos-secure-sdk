...
cx_err_t cx_blake3_update(cx_blake3_t *hash, const void *input, size_t input_len)
{
    ...
    while (input_len > BLAKE3_CHUNK_LEN) {
        ...
        if (subtree_len <= BLAKE3_CHUNK_LEN) {
            ...
            blake3_compress_subtree_to_parent(
                input_bytes, subtree_len, hash->key, (hash->chunk).t, (hash->chunk).d, cv_pair);
            blake3_hasher_push_cv(hash, cv_pair, (hash->chunk).t);
            blake3_hasher_push_cv(
                hash, cv_pair + BLAKE3_OUT_LEN, (hash->chunk).t + (subtree_chunks / 2));
        }
...
cx_err_t cx_blake3_final(cx_blake3_t *hash, uint8_t *output, size_t out_len)
{
    ...
    size_t state_len
        = BLAKE3_BLOCK_LEN * (hash->chunk).blocks_compressed + (hash->chunk).buffer_len;

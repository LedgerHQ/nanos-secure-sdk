...
cx_err_t des_setkey(cx_des_key_t  *ctx_key,
                    uint32_t       operation,
                    const uint8_t *key,
                    uint32_t       key_bitlen)
{
...
static const cx_cipher_base_t des_base
    = {(cx_err_t(*)(const cipher_key_t *ctx_key, const uint8_t *inblock, uint8_t *outblock))
       ...
       (cx_err_t(*)(const cipher_key_t *ctx_key,
                    uint32_t            operation,
                    const uint8_t      *key,
                    uint32_t            key_bitlen)) des_setkey,
       (cx_err_t(*)(void)) cx_des_reset_hw};

const cx_cipher_info_t cx_des_64_info = {CX_DES_KEY_LENGTH, 8, CX_DES_BLOCK_SIZE, &des_base};

const cx_cipher_info_t cx_3des_128_info = {CX_3DES_2_KEY_LENGTH, 8, CX_DES_BLOCK_SIZE, &des_base};

const cx_cipher_info_t cx_3des_192_info = {CX_3DES_3_KEY_LENGTH, 8, CX_DES_BLOCK_SIZE, &des_base};

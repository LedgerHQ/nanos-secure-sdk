...
/* ----------------------------------------------------------------------- */
static size_t cx_rfc6979_bits2int(cx_rnd_rfc6979_ctx_t *rfc_ctx, const uint8_t *b, size_t b_len, uint8_t *b_out) {
...
/*    b_len: bits length os b, shall be multiple of 8, true by design      */
/* ----------------------------------------------------------------------- */
static void cx_rfc6979_bits2octets(cx_rnd_rfc6979_ctx_t *rfc_ctx, const uint8_t *b, size_t b_len, uint8_t *b_out) {
...
/* ----------------------------------------------------------------------- */
static void cx_rfc6979_int2octets(cx_rnd_rfc6979_ctx_t *rfc_ctx, const uint8_t *i, size_t i_len, uint8_t *b_out) {
  int32_t delta;
...
static void cx_rfc6979_hmacVK(cx_rnd_rfc6979_ctx_t *rfc_ctx,
                         int8_t opt,
...
cx_err_t cx_rng_rfc6979_init(cx_rnd_rfc6979_ctx_t *rfc_ctx, cx_md_t hash_id,
                             const uint8_t *x, size_t x_len,
                             const uint8_t *h1, size_t h1_len,
  ...
  cx_rfc6979_hmacVK(rfc_ctx, 0, x, x_len, h1, h1_len, rfc_ctx->k /*,  additional_input, additional_input_len*/);

  //Step E: V = HMAC (K, V).
  cx_rfc6979_hmacVK(rfc_ctx, -1, NULL, 0, NULL, 0, rfc_ctx->v);

  //Step F:  K = HMAC (K, V || 0x01 || int2octets(x) || bits2octetc(h1) [ || additional_input])
  cx_rfc6979_hmacVK(rfc_ctx, 0x01, x, x_len, h1, h1_len, rfc_ctx->k /*,  additional_input, additional_input_len*/);

...
typedef enum {
    ...
    NORMAL_INFO   ///< Icon in black, a possible text in black under it, and a possible text in dark
                  ///< gray under it
...
/**
 * @brief This structure contains info to build a pair of buttons, button1 on top of button2, with a
 * simple choice. If the second text and icon2 are NULL, a single button will be used
 * @note both icons must have the same dimensions (32*32px)
 * @note the pair of button is automatically put on bottom of screen if not the only objects, and
 * centered otherwise
 */
typedef struct {

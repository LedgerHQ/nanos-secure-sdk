 ...
 *
 * @note this type must never be instanciated
 */
typedef struct PACKED__ nbgl_obj_s {
    struct nbgl_area_s;    ///< absolute position, backGround color and size of the object. DO NOT
                           ///< MOVE THIS FIELD
    nbgl_obj_type_t type;  ///< type of the graphical object, must be set explicitely
    int16_t
    ...
    nbgl_touchCallback_t
        touchCallback;  ///< function to be called on events defined in @ref touchMask
} nbgl_obj_t;

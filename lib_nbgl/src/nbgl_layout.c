...
#define GET_NEXT_OBJ_FROM_POOL(__layout, __obj)                                   \
    {                                                                             \
        if ((__layout)->nbUsedCallbackObjs < (LAYOUT_OBJ_POOL_LEN - 1)) {         \
            __obj = &(__layout)->callbackObjPool[(__layout)->nbUsedCallbackObjs]; \
            (__layout)->nbUsedCallbackObjs++;                                     \
        }                                                                         \
        else                                                                      \
                                  ...
                                  layoutObj_t           **layoutObj)
{
    ...
    LOG_DEBUG(LAYOUT_LOGGER,
              "touchCallback(): i = %d, nbUsedCallbackObjs = %d\n",
              i,
...
static void touchCallback(nbgl_obj_t *obj, nbgl_touchType_t eventType)
{
    ...
    UNUSED(eventType);
    LOG_DEBUG(LAYOUT_LOGGER,
              "touchCallback(): obj = %p, gLayout[1].nbChildren = %d\n",
              obj,
    ...
    if ((obj == (nbgl_obj_t *) layout->bottomContainer)
...
nbgl_layout_t *nbgl_layoutGet(nbgl_layoutDescription_t *description)
{
    ...
    if (description->modal) {
        layout->layer
            = nbgl_screenPush(&layout->children, NB_MAX_SCREEN_CHILDREN, &description->ticker);
    }
    else {
...
int nbgl_layoutAddNavigationBar(nbgl_layout_t *layout, nbgl_layoutNavigationBar_t *info)
{
    ...
    layoutInt->bottomContainer                   = nbgl_navigationPopulate(info->nbPages,
                                                         info->activePage,
                                                         info->withExitKey,
                               ...
                               tune_index_e               tuneId)
{
    ...
    GET_NEXT_OBJ_FROM_POOL((nbgl_layoutInternal_t *) layout, obj);
    layoutInt->bottomContainer = nbgl_bottomButtonPopulate(
        icon, (nbgl_touchCallback_t) &touchCallback, separationLine, layoutInt->layer);
    obj->obj                                     = (nbgl_obj_t *) layoutInt->bottomContainer;
    obj->token                                   = token;
    obj->tuneId                                  = tuneId;
    layoutInt->bottomContainer->alignmentMarginX = 0;
...
int nbgl_layoutAddText(nbgl_layout_t *layout, char *text, char *subText)
{
    ...
    if (subText != NULL) {
        ...
        subTextArea->width     = container->width;
        subTextArea->height
            = nbgl_getTextHeightInWidth(subTextArea->fontId, subTextArea->text, subTextArea->width);
        subTextArea->textAlignment    = MID_LEFT;
        subTextArea->alignment        = NO_ALIGNMENT;
        subTextArea->alignTo          = NULL;
        subTextArea->alignmentMarginX = 0;
...
int nbgl_layoutAddCenteredInfo(nbgl_layout_t *layout, nbgl_layoutCenteredInfo_t *info)
{
    ...
    if (info->text1 != NULL) {
        ...
        textArea->width = GET_AVAILABLE_WIDTH(layoutInt);
        textArea->height
            = nbgl_getTextHeightInWidth(textArea->fontId, textArea->text, textArea->width);
...
int nbgl_layoutAddQRCode(nbgl_layout_t *layout, nbgl_layoutQRCode_t *info)
{
    ...
    if (info->text1 != NULL) {
        ...
        textArea->text          = PIC(info->text1);
        textArea->textAlignment = CENTER;
        textArea->fontId        = (info->largeText1 == true) ? BAGL_FONT_INTER_REGULAR_32px
...
int nbgl_layoutAddTagValueList(nbgl_layout_t *layout, nbgl_layoutTagValueList_t *list)
{
    ...
    for (i = 0; i < list->nbPairs; i++) {
        ...
        itemTextArea->height
            = nbgl_getTextHeightInWidth(itemTextArea->fontId, itemTextArea->text, usableWidth);
        ...
        uint16_t nbLines
            = nbgl_getTextNbLinesInWidth(valueTextArea->fontId, valueTextArea->text, usableWidth);
...
int nbgl_layoutAddButton(nbgl_layout_t *layout, nbgl_layoutButton_t *buttonInfo)
{
    ...
    if (buttonInfo->fittingContent == true) {
        button->width = nbgl_getTextWidth(button->fontId, button->text) + 64
...
/**
 * @brief Creates a kind of navigation bar with an optionnal <- arrow on the left. This widget is

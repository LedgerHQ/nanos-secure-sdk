...
static void draw_image(nbgl_image_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    if (!iconDetails->isFile) {
        nbgl_frontDrawImage(
            (nbgl_area_t *) obj, (uint8_t *) iconDetails->bitmap, NO_TRANSFORMATION, colorMap);
    }
    else {
        nbgl_frontDrawImageFile(
            (nbgl_area_t *) obj, (uint8_t *) iconDetails->bitmap, colorMap, ramBuffer);
    }
}
...
static void draw_radioButton(nbgl_radio_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    if (obj->state == OFF_STATE) {
        nbgl_frontDrawImage(&rectArea,
                            (uint8_t *) C_radio_inactive_32px.bitmap,
                            NO_TRANSFORMATION,
                            obj->borderColor);
    }
    else {
        nbgl_frontDrawImage(
            &rectArea, (uint8_t *) C_radio_active_32px.bitmap, NO_TRANSFORMATION, obj->activeColor);
    }
}
...
static void draw_textArea(nbgl_text_area_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    for (line = 0; line < nbLines; line++) {
        ...
        nbgl_getTextMaxLenAndWidth(
            obj->fontId, text, obj->width, &lineLen, &lineWidth, obj->wrapping);
        ...
        LOG_DEBUG(OBJ_LOGGER,
                  "draw_textArea(), %s line %d, lineLen %d lineWidth = %d, obj->height = %d, "
                  "textHeight = %d, obj->nbMaxLines = %d\n",
                  text,
                  line,
                  lineLen,
                  lineWidth,

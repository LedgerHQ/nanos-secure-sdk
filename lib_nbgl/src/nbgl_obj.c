        ...
        if (alignToObj == obj->parent) {
            ...
            switch (obj->alignment) {
                case TOP_LEFT:
                    ...
                    break;
                case TOP_MIDDLE:
                    obj->rel_x0 = alignToObj->rel_x0 + (alignToObj->width - obj->width) / 2
                                  + obj->alignmentMarginX;
                    obj->rel_y0 = alignToObj->rel_y0 - obj->height - obj->alignmentMarginY;
                    break;
                case TOP_RIGHT:
                    obj->rel_x0 = alignToObj->rel_x0 + (alignToObj->width - obj->width)
                                  - obj->alignmentMarginX;
                    obj->rel_y0 = alignToObj->rel_y0 - obj->height - obj->alignmentMarginY;
                    break;
                ...
                case MID_LEFT:
                    obj->rel_x0 = alignToObj->rel_x0 - obj->width - obj->alignmentMarginX;
                    obj->rel_y0 = alignToObj->rel_y0 + (alignToObj->height - obj->height) / 2
                                  + obj->alignmentMarginY;
                    break;
                case LEFT_BOTTOM:
                    obj->rel_x0 = alignToObj->rel_x0 - obj->width - obj->alignmentMarginX;
                    obj->rel_y0 = alignToObj->rel_y0 + (alignToObj->height - obj->height)
                                  + obj->alignmentMarginY;
                    break;
...
static void compute_position(nbgl_obj_t *obj, nbgl_obj_t *prevObj)
{
    ...
    // LOG_DEBUG(OBJ_LOGGER,"compute_position(), parent.type = %d, parent->x0 = %d,
    // obj->rel_x0=%d\n",parent->type,parent->x0,obj->rel_x0);
    // LOG_DEBUG(OBJ_LOGGER,"compute_position(), parent->y0 = %d,
...
static void draw_panel(nbgl_panel_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    nbgl_drawRoundedBorderedRect((nbgl_area_t *) obj, RADIUS_8_PIXELS, 2, WHITE, obj->borderColor);
}
...
static void draw_button(nbgl_button_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    if (text != NULL) {
        ...
        if (obj->icon != NULL) {
            rectArea.x0 = obj->x0 + obj->width / 2 - (textWidth + obj->icon->width + 8) / 2
                          + obj->icon->width + 8;
        }
        else {
    ...
    if (obj->icon != NULL) {
        ...
        LOG_DEBUG(OBJ_LOGGER,
                  "draw_button(), obj->height = %d, obj->iconHeight = %d\n",
                  obj->height,
                  obj->icon->height);
        ...
        nbgl_frontDrawImage(
            &rectArea, (uint8_t *) obj->icon->bitmap, NO_TRANSFORMATION, obj->foregroundColor);
    }
}
...
static void draw_image(nbgl_image_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    if (obj->bpp == NBGL_BPP_1) {
        nbgl_frontDrawImage((nbgl_area_t *) obj,
                            (uint8_t *) iconDetails->bitmap,
                            NO_TRANSFORMATION,
                            obj->foregroundColor);
    }
        ...
        nbgl_frontDrawImage((nbgl_area_t *) obj,
                            (uint8_t *) iconDetails->bitmap,
                            NO_TRANSFORMATION,
                            ((WHITE << 6) | (LIGHT_GRAY << 4) | (DARK_GRAY << 2) | BLACK));
    }
    else {
        nbgl_frontDrawImage((nbgl_area_t *) obj,
                            (uint8_t *) iconDetails->bitmap,
...
static void draw_radioButton(nbgl_radio_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    if (obj->state == OFF_STATE) {
        nbgl_frontDrawImage(
            &rectArea, (uint8_t *) C_radio_inactive_32px.bitmap, NO_TRANSFORMATION, LIGHT_GRAY);
    }
    else {
        nbgl_frontDrawImage(
            &rectArea, (uint8_t *) C_radio_active_32px.bitmap, NO_TRANSFORMATION, BLACK);
    }
}
...
static void draw_progressBar(nbgl_progress_bar_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    if (obj->withBorder) {
        ...
        nbgl_drawRoundedRect((nbgl_area_t *) obj, RADIUS_0_PIXELS, obj->backgroundColor);
    }
    // draw level
...
#define INTER_DASHES 4  // pixels
        ...
        LOG_DEBUG(OBJ_LOGGER,
                  "draw_navigationBar(), x0 = %d, y0 = %d, page = %d/%d\n",
                  obj->x0,
                  obj->y0,
                  obj->activePage,
                  obj->nbPages);
...
static void draw_textArea(nbgl_text_area_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    LOG_DEBUG(OBJ_LOGGER,
              "draw_textArea(), x0 = %d, y0 = %d, width = %d, height = %d, text = %s\n",
              obj->x0,
              obj->y0,
              obj->width,
              obj->height,
    ...
    for (line = 0; line < nbLines; line++) {
        ...
        if (textWidth > obj->width) {
            if (obj->autoHideLongLine == false) {
                ...
                LOG_DEBUG(OBJ_LOGGER,
                          "draw_textArea(), line %d, textWidth = %d, fullLen = %d\n",
                          line,
                          textWidth,
                          fullLen);
                ...
                while (fullLen > 0) {
                    ...
                    nbgl_getTextMaxLenAndWidth(
                        obj->fontId, tmpText, obj->width, &lineLen, &lineWidth);
                    if (lineLen == 0) {
                        break;
                    }
                    tmpText += lineLen;
                ...
                LOG_DEBUG(OBJ_LOGGER,
                          "draw_textArea(), line %d, textWidth = %d, nbChunks = %d\n",
                          line,
                          textWidth,
                          nbChunks);
                ...
                while (chunk < nbChunks) {
                    ...
                    if (obj->textAlignment == MID_LEFT) {
                        ...
                        LOG_FATAL(
                            OBJ_LOGGER, "Forbidden obj->textAlignment = %d\n", obj->textAlignment);
                    }
                    rectArea.x0    = textX0;
                    rectArea.y0    = textY0;
                    ...
                    if ((obj->nbMaxLines == 0) || (chunk < (obj->nbMaxLines - 1))) {
                        ...
                        rectArea.x0
                            += nbgl_getSingleLineTextWidthInLen(obj->fontId, tmpText, lineLen - 3);
                ...
                nbgl_getTextMaxLenAndWidthFromEnd(
                    obj->fontId, text, obj->width - dotsWidth, &lineLen, &lineWidth);
...
void nbgl_redrawObject(nbgl_obj_t *obj, nbgl_obj_t *prevObj, bool computePosition)
{
    ...
    if ((obj->type == SCREEN) || (obj->type == CONTAINER) || (obj->type == PANEL)) {

...
static void keyboardTouchCallback(nbgl_obj_t *obj, nbgl_touchType_t eventType)
{
    ...
    if (keyboard->mode == MODE_LETTERS) {
        if ((firstIndex < 26) && ((keyboard->keyMask & (1 << firstIndex)) == 0)) {
            keyboard->callback(keyboard->upperCase ? kbd_chars_upper[firstIndex]
...
static void keyboardDrawLetters(nbgl_keyboard_t *keyboard)
{
    ...
    for (i = 0; i < 10; i++) {
        ...
        rectArea.x0
            += (NORMAL_KEY_WIDTH - nbgl_getCharWidth(BAGL_FONT_INTER_REGULAR_24px, &keys[i])) / 2;
    ...
    if (!keyboard->lettersOnly) {
        ...
        rectArea.bpp    = NBGL_BPP_1;
        rectArea.y0
            = keyboard->y0 + KEYBOARD_KEY_HEIGHT * 2 + (KEYBOARD_KEY_HEIGHT - rectArea.height) / 2;
        rectArea.x0              = (SHIFT_KEY_WIDTH - rectArea.width) / 2;
        rectArea.backgroundColor = (keyboard->upperCase) ? BLACK : WHITE;
        nbgl_frontDrawImage(&rectArea,
                            (uint8_t *) C_shift32px.bitmap,
                            NO_TRANSFORMATION,
                            (keyboard->upperCase) ? WHITE : BLACK);
    ...
    if (!keyboard->lettersOnly) {
        rectArea.x0
            = (SWITCH_KEY_WIDTH - nbgl_getTextWidth(BAGL_FONT_INTER_REGULAR_24px, ".?123")) / 2;
        ...
        rectArea.x0
            = SWITCH_KEY_WIDTH
              + (SPACE_KEY_WIDTH - nbgl_getTextWidth(BAGL_FONT_INTER_REGULAR_24px, "space")) / 2;
        nbgl_drawText(&rectArea,
                      "space",
                      5,
                      BAGL_FONT_INTER_REGULAR_24px,
...
static void keyboardDrawDigits(nbgl_keyboard_t *keyboard)
{
    ...
    if (keyboard->mode == MODE_DIGITS) {
        rectArea.x0
            = (SPECIAL_CHARS_KEY_WIDTH - nbgl_getTextWidth(BAGL_FONT_INTER_REGULAR_24px, "#+="))
              / 2;
        rectArea.y0 = keyboard->y0 + KEYBOARD_KEY_HEIGHT * 2 + LETTER_OFFSET_Y;
        nbgl_drawText(&rectArea, "#+=", 3, BAGL_FONT_INTER_REGULAR_24px, BLACK);
    }
    else {
        rectArea.x0
            = (SPECIAL_CHARS_KEY_WIDTH - nbgl_getTextWidth(BAGL_FONT_INTER_REGULAR_24px, "123"))
              / 2;
        rectArea.y0 = keyboard->y0 + KEYBOARD_KEY_HEIGHT * 2 + LETTER_OFFSET_Y;
        nbgl_drawText(&rectArea, "123", 3, BAGL_FONT_INTER_REGULAR_24px, BLACK);
    }
    ...
    // 4th row
    rectArea.x0 = (SWITCH_KEY_WIDTH - nbgl_getTextWidth(BAGL_FONT_INTER_REGULAR_24px, "ABC")) / 2;
    rectArea.y0 = keyboard->y0 + KEYBOARD_KEY_HEIGHT * 3 + LETTER_OFFSET_Y;

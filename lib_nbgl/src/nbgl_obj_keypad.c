...
static void keypadDrawGrid(nbgl_keypad_t *keypad) {
  ...
  // clean full area
  rectArea.backgroundColor = keypad->area.backgroundColor;
  rectArea.x0 = keypad->area.x0;
  rectArea.y0 = keypad->area.y0;
  rectArea.width = keypad->area.width;
  rectArea.height = keypad->area.height;
...
static void keypadDrawDigits(nbgl_keypad_t *keypad) {
  ...
  rectArea.backgroundColor = keypad->area.backgroundColor;
  rectArea.y0 = keypad->area.y0 + DIGIT_OFFSET_Y;

  // First row of keys: 1 2 3
  for (i=0;i<3;i++) {
    key_value = GET_DIGIT_INDEX(keypad,(i+1)) + 0x30;

    rectArea.x0 = keypad->area.x0 + i*KEY_WIDTH;
  ...
  for (;i<6;i++) {
    key_value = GET_DIGIT_INDEX(keypad,(i+1)) + 0x30;
    rectArea.x0 = keypad->area.x0 + (i-3)*KEY_WIDTH;
    rectArea.x0 += (KEY_WIDTH-nbgl_getCharWidth(BAGL_FONT_HM_ALPHA_MONO_MEDIUM_32px,&key_value))/2;
    nbgl_drawText(&rectArea, &key_value,
                  1, BAGL_FONT_HM_ALPHA_MONO_MEDIUM_32px,
                  keypad->enableDigits ? BLACK :  WHITE);
  ...
  for (;i<9;i++) {
    key_value = GET_DIGIT_INDEX(keypad,(i+1)) + 0x30;
    rectArea.x0 = keypad->area.x0 + (i-6)*KEY_WIDTH;
    rectArea.x0 += (KEY_WIDTH-nbgl_getCharWidth(BAGL_FONT_HM_ALPHA_MONO_MEDIUM_32px,&key_value))/2;
    nbgl_drawText(&rectArea, &key_value,
                  1, BAGL_FONT_HM_ALPHA_MONO_MEDIUM_32px,
                  keypad->enableDigits ? BLACK :  WHITE);
  ...
  rectArea.x0 = keypad->area.x0 + (KEY_WIDTH-rectArea.width)/2;
  rectArea.y0 = keypad->area.y0 + KEYPAD_KEY_HEIGHT*3 + (KEYPAD_KEY_HEIGHT-rectArea.height)/2;
  ...
  rectArea.x0 = keypad->area.x0 + KEY_WIDTH;
  rectArea.x0 += (KEY_WIDTH-nbgl_getCharWidth(BAGL_FONT_HM_ALPHA_MONO_MEDIUM_32px,&key_value))/2;
  rectArea.y0 = keypad->area.y0 + KEYPAD_KEY_HEIGHT*3 + DIGIT_OFFSET_Y;
  nbgl_drawText(&rectArea, &key_value,
                1, BAGL_FONT_HM_ALPHA_MONO_MEDIUM_32px,
                keypad->enableDigits ? BLACK :  WHITE);
  ...
  if (!keypad->enableValidate) {
    ...
    rectArea.bpp = NBGL_BPP_1;
    rectArea.x0 = keypad->area.x0 + 2*KEY_WIDTH + (KEY_WIDTH-rectArea.width)/2;
    rectArea.y0 = keypad->area.y0 + KEYPAD_KEY_HEIGHT*3 + (KEYPAD_KEY_HEIGHT-rectArea.height)/2;
    ...
    rectArea.x0 = keypad->area.x0 + 2*KEY_WIDTH;
    rectArea.y0 = keypad->area.y0 + KEYPAD_KEY_HEIGHT*3;

    ...
    switch (content->type) {
        case INFO_LONG_PRESS: {
            ...
            nbgl_layoutAddLongPressButton(layout,
                                          (char *) content->infoLongPress.longPressText,
        ...
        case INFOS_LIST: {
            ...
            for (i = 0; i < content->infosList.nbInfos; i++) {
                nbgl_layoutAddText(layout,
                                   (char *) content->infosList.infoTypes[i],
                                   (char *) content->infosList.infoContents[i]);
                nbgl_layoutAddSeparationLine(layout);
            }
            break;
        }
...
static void displayQR(void)
{
    nbgl_layoutDescription_t layoutDescription
        = {.modal = true, .onActionCallback = &layoutTouchCallbackQR, .tapActionText = NULL};
                                     ...
                                     const char                       *text)
{
    nbgl_layoutDescription_t  layoutDescription;
    nbgl_layout_t            *layout;
    nbgl_layoutCenteredInfo_t centeredInfo = {.text1   = (char *) text,
                                              .text2   = NULL,
                                              .text3   = NULL,
                                              .style   = LEDGER_INFO,
                                              .icon    = NULL,
                               ...
                               nbgl_pageInfoDescription_t       *info)
{
    ...
    if (info->footerText != NULL) {
        nbgl_layoutAddFooter(
            layout, (char *) PIC(info->footerText), info->footerToken, info->tuneId);
    }
                                       ...
                                       nbgl_pageConfirmationDescription_t *info)
{
    ...
    if (info->cancelText == NULL) {
        ...
        nbgl_layoutChoiceButtons_t buttonsInfo = {.bottomText = (char *) PIC(info->cancelText),
                                                  .token      = info->confirmationToken,
 ...
 *
 * @param onActionCallback common callback for all actions on this page (confirmation and
 * cancelation)
 * @param info structure describing the all controls of this page
 * @return the page context (or NULL if error)
 */
nbgl_page_t *nbgl_pageDrawAddressConfirmation(nbgl_layoutTouchCallback_t onActionCallback,
                                              nbgl_pageAddressConfirmationDescription_t *info)
{
    nbgl_layoutButton_t      buttonInfo;
    nbgl_layoutDescription_t layoutDescription
        = {.modal = false, .onActionCallback = &layoutTouchCallback, .tapActionText = NULL};
    uint8_t pageModeToken;

    // save context
    addressConfirmationContext.callback = onActionCallback;
    memcpy(&addressConfirmationContext.description,
           info,
           sizeof(nbgl_pageAddressConfirmationDescription_t));
    ...
    if (addressConfirmationContext.description.cancelText != NULL) {
        nbgl_layoutAddFooter(addressConfirmationContext.layout,
                             (char *) addressConfirmationContext.description.cancelText,
                             addressConfirmationContext.description.cancelToken,
                             TUNE_TAP_CASUAL);
    }
    else {
        nbgl_layoutAddFooter(addressConfirmationContext.layout,
    ...
    nbgl_layoutTagValue_t tagValuePair
        = {.item = "Address", .value = (char *) addressConfirmationContext.description.address};
                                            ...
                                            bool                       modal)
{
    ...
    if (nav != NULL) {
        if (nav->navType == NAV_WITH_TAP) {
            nbgl_layoutAddFooter(
                layout, (char *) nav->navWithTap.quitText, nav->quitToken, nav->tuneId);

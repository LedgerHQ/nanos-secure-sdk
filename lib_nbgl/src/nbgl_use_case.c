...
static void pageCallback(int token, uint8_t index)
{
    ...
    if (token == QUIT_TOKEN) {
        ...
        nbgl_useCaseViewDetails(
            staticReviewContext.tagValueList->pairs[staticReviewContext.currentPairIndex].item,
...
static void displayStaticReviewPage(uint8_t page)
{
    ...
    // if it's the last page, display a long press button with the info provided by user (except
    // token)
    if (page == (navInfo.nbPages - 1)) {
        ...
        if ((page != (navInfo.nbPages - 2)) || (page > navInfo.activePage)) {
            // we have to recompute the number of pairs to display in the page, and the index in
            // which to start
        ...
        staticReviewContext.nbPairsInCurrentPage = getNbTagValuesInPage(
            forward
                ? staticReviewContext.tagValueList->nbPairs - staticReviewContext.currentPairIndex
...
static void displayDetailsPage(uint8_t detailsPage)
{
    ...
    uint16_t nbLines           = nbgl_getTextNbLinesInWidth(
        BAGL_FONT_INTER_REGULAR_24px, currentPair.value, SCREEN_WIDTH - 2 * BORDER_MARGIN);
                                    ...
                                    bool                  *tooLongToFit)
{
    ...
    *tooLongToFit = false;
    while (nbPairsInPage < nbPairs) {
        ...
        currentHeight
            += nbgl_getTextHeightInWidth(BAGL_FONT_INTER_REGULAR_32px,
                                         pairs[forward ? nbPairsInPage : -nbPairsInPage - 1].value,
                                         SCREEN_WIDTH - 2 * BORDER_MARGIN);  // value height
 ...
 * @param tagline text under app name (if NULL, it will be "This app confirms actions on the
 * <appName> network.")
                      ...
                      nbgl_callback_t            quitCallback)
{
    ...
    if (tagline == NULL) {
        snprintf(appDescription,
                 ...
                 "This app confirms actions on the %s network.",
                 appName);
 ...
 */
void nbgl_useCaseSettings(char                      *title,
                          uint8_t                    initPage,
                          uint8_t                    nbPages,
                          bool                       touchable,
                          nbgl_callback_t            quitCallback,
 ...
 * thanks to a button and a footer at the bottom of the page. The given callback is called with true
 * as argument if the button is touched, false if footer is touched
 *
...
{
    nbgl_pageConfirmationDescription_t info = {.cancelToken          = CHOICE_TOKEN,
 ...
 * @param choiceCallback callback called when either long_press or footer is called (parm is true
 * for long press)
 */
...
{
    uint16_t nbLines = nbgl_getTextNbLinesInWidth(
        BAGL_FONT_INTER_REGULAR_24px, value, SCREEN_WIDTH - 2 * BORDER_MARGIN);
...
/**
 * @brief draw an address confirmation page. This page contains a "page selector" to display either
 * address in text or QRCode format, and at the bottom a button to confirm and a footer to cancel
 *
...
void nbgl_useCaseAddressConfirmation(char *address, nbgl_choiceCallback_t callback)
{
    ...
    nbgl_layoutDescription_t layoutDescription
        = {.modal = false, .onActionCallback = &addressLayoutTouchCallback, .tapActionText = NULL};
    ...
    nbgl_layoutAddFooter(
        addressConfirmationContext.layout, "It doesn't match", QUIT_TOKEN, TUNE_TAP_CASUAL);

    nbgl_layoutTagValue_t     tagValuePair = {.item = "Address", .value = (char *) address};
    nbgl_layoutTagValueList_t tagValueList
        = {.nbPairs = 1, .pairs = &tagValuePair, .smallCaseForValue = false};

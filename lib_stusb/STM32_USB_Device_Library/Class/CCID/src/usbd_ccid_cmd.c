...
#define CCID_UpdateCommandStatus(cmd_status,icc_status)\
 Ccid_bulk_data.header.bulkin.bStatus=(cmd_status|icc_status)
  /* 
...
uint8_t PC_to_RDR_IccPowerOn(void)
{
  ...
  */
  ...
  Ccid_bulk_data.header.bulkin.dwLength = 0;  /* Reset Number of Bytes in abData */
  ...
  03h – 1.8 volts
  */
  /* Ccid_bulk_data.header.bulkout.bSpecific_0 Contains bPowerSelect */
  voltage = Ccid_bulk_data.header.bulkout.bSpecific_0;
  ...
  Ccid_bulk_data.header.bulkin.dwLength = SC_AnswerToReset(sc_voltage, Ccid_bulk_data.abData);
...
uint8_t  PC_to_RDR_XfrBlock(void)
{
  ...
  if (error != 0) 
    ...
    /* wLevelParameter = Size of expected data to be returned by the 
                          bulk-IN endpoint */
    expectedLength = (Ccid_bulk_data.header.bulkout.bSpecific_2 << 8) | 
                      Ccid_bulk_data.header.bulkout.bSpecific_1;   

    reqlen = Ccid_bulk_data.header.bulkout.dwLength;
    
    Ccid_bulk_data.header.bulkin.dwLength = (uint16_t)expectedLength;  
...
uint8_t  PC_to_RDR_ResetParameters(void)
{
  ...
  Ccid_bulk_data.abData[0] = DEFAULT_FIDI;
  Ccid_bulk_data.abData[1] = DEFAULT_T01CONVCHECKSUM;
  Ccid_bulk_data.abData[2] = DEFAULT_EXTRA_GUARDTIME;
  Ccid_bulk_data.abData[3] = DEFAULT_WAITINGINTEGER;
  Ccid_bulk_data.abData[4] = DEFAULT_CLOCKSTOP;
  Ccid_bulk_data.abData[5] = 0x00;
  Ccid_bulk_data.abData[6] = 0x00;
  
  memmove(&Protocol0_DataStructure, (Protocol0_DataStructure_t*)(&(Ccid_bulk_data.abData[0])), sizeof(Protocol0_DataStructure_t));
  error = SC_SetParams((Protocol0_DataStructure_t*)(&(Ccid_bulk_data.abData[0])));
...
uint8_t  PC_to_RDR_SetParameters(void)
{
  ...
  if ( (Ccid_bulk_data.header.bulkout.dwLength == 5) &&
      (Ccid_bulk_data.header.bulkout.bSpecific_0 != 0))
    error = SLOTERROR_BAD_PROTOCOLNUM;
  
  /* for Protocol T=1 (bProtocolNum=1) (dwLength=00000007h) */
  if ( (Ccid_bulk_data.header.bulkout.dwLength == 7) &&
      (Ccid_bulk_data.header.bulkout.bSpecific_0 != 1))
    error = SLOTERROR_CMD_NOT_SUPPORTED;
    
  /* For T0, Waiting Integer 0 supported */
  if (Ccid_bulk_data.abData[3] != 0)
    error = SLOTERROR_BAD_WAITINGINTEGER;
      
  if (Ccid_bulk_data.abData[4] != DEFAULT_CLOCKSTOP)
...
uint8_t  PC_to_RDR_Escape(void)
{
  ...
  error = SC_ExecuteEscape(&Ccid_bulk_data.abData[0],
                           Ccid_bulk_data.header.bulkout.dwLength,
                           &Ccid_bulk_data.abData[0],
                           &size);
  
  Ccid_bulk_data.header.bulkin.dwLength = size;
...
uint8_t  PC_to_RDR_IccClock(void)
{
  ...
  if (Ccid_bulk_data.header.bulkout.bSpecific_0 > 1)
  {
    CCID_UpdateCommandStatus(BM_COMMAND_STATUS_FAILED, BM_ICC_PRESENT_ACTIVE);
    return SLOTERROR_BAD_CLOCKCOMMAND;
  }
  
  error = SC_SetClock(Ccid_bulk_data.header.bulkout.bSpecific_0);
...
uint8_t  PC_to_RDR_Abort(void)
{
  ...
  CCID_CmdAbort (Ccid_bulk_data.header.bulkout.bSlot, Ccid_bulk_data.header.bulkout.bSeq);
  CCID_UpdateCommandStatus(BM_COMMAND_STATUS_NO_ERROR,BM_ICC_PRESENT_ACTIVE);               
  return SLOT_NO_ERROR;
}
...
uint8_t  PC_TO_RDR_T0Apdu(void)
{
  ...
  if (Ccid_bulk_data.header.bulkout.bSpecific_0 > 0x03)
  ...
  error = SC_T0Apdu(Ccid_bulk_data.header.bulkout.bSpecific_0, 
                    Ccid_bulk_data.header.bulkout.bSpecific_1, 
                    Ccid_bulk_data.header.bulkout.bSpecific_2);
...
uint8_t  PC_TO_RDR_Mechanical(void)
{
  ...
  if (Ccid_bulk_data.header.bulkout.bSpecific_0 > 0x05)
  {/* 01h – Accept Card
      02h – Eject Card
      03h – Capture Card
      04h – Lock Card
  ...
  error = SC_Mechanical(Ccid_bulk_data.header.bulkout.bSpecific_0);
...
uint8_t  PC_TO_RDR_SetDataRateAndClockFrequency(void)
{
  ...
  if (Ccid_bulk_data.header.bulkout.dwLength != 0x08)
  {
    CCID_UpdateCommandStatus(BM_COMMAND_STATUS_FAILED,BM_ICC_PRESENT_ACTIVE);
  ...
  temp = (Ccid_bulk_data.abData[0]) & 0x000000FF;
  clockFrequency = temp;
  
  temp= (Ccid_bulk_data.abData[1]) & 0x000000FF;
  clockFrequency |= temp << 8;
  
  temp= (Ccid_bulk_data.abData[2]) & 0x000000FF;
  clockFrequency |= temp << 16;
  
  temp= (Ccid_bulk_data.abData[3]) & 0x000000FF;
  clockFrequency |= temp << 24;
  
  temp = (Ccid_bulk_data.abData[4]) & 0x000000FF;
  dataRate = temp;
  
  temp= (Ccid_bulk_data.abData[5]) & 0x000000FF;
  dataRate |= temp << 8;
  
  temp= (Ccid_bulk_data.abData[6]) & 0x000000FF;
  dataRate |= temp << 16;
  
  temp= (Ccid_bulk_data.abData[7]) & 0x000000FF;
  dataRate |= temp << 24;
  
  error = SC_SetDataRateAndClockFrequency(clockFrequency, dataRate);
  Ccid_bulk_data.header.bulkin.bError = error;
  ...
  {
    Ccid_bulk_data.header.bulkin.dwLength = 8;
    
    (Ccid_bulk_data.abData[0])  = clockFrequency & 0x000000FF ;
    
    (Ccid_bulk_data.abData[1])  =  (clockFrequency & 0x0000FF00) >> 8;
    
    (Ccid_bulk_data.abData[2])  =  (clockFrequency & 0x00FF0000) >> 16;
    
    (Ccid_bulk_data.abData[3])  =  (clockFrequency & 0xFF000000) >> 24;
    
    (Ccid_bulk_data.abData[4])  =  dataRate & 0x000000FF ;
    
    (Ccid_bulk_data.abData[5])  =  (dataRate & 0x0000FF00) >> 8;
    
    (Ccid_bulk_data.abData[6])  =  (dataRate & 0x00FF0000) >> 16;
    
    (Ccid_bulk_data.abData[7])  =  (dataRate & 0xFF000000) >> 24;
...
uint8_t  PC_TO_RDR_Secure(void)
{
  ...
  bBWI = Ccid_bulk_data.header.bulkout.bSpecific_0;
  wLevelParameter = (Ccid_bulk_data.header.bulkout.bSpecific_1 + ((uint16_t)Ccid_bulk_data.header.bulkout.bSpecific_2<<8));
  ...
  error = SC_Secure(Ccid_bulk_data.header.bulkout.dwLength, bBWI, wLevelParameter, 
                    &Ccid_bulk_data.abData[0], &responseLen);

  Ccid_bulk_data.header.bulkin.dwLength = responseLen;
...
{  
  Ccid_bulk_data.header.bulkin.bMessageType = RDR_TO_PC_DATABLOCK; 
  Ccid_bulk_data.header.bulkin.bError = errorCode; 
  Ccid_bulk_data.header.bulkin.bSpecific=0;    /* bChainParameter */
...
void RDR_to_PC_SlotStatus(uint8_t  errorCode)
{
  ...
  Ccid_bulk_data.header.bulkin.bMessageType = RDR_TO_PC_SLOTSTATUS; 
  Ccid_bulk_data.header.bulkin.dwLength =0;
  Ccid_bulk_data.header.bulkin.bError = errorCode; 
  Ccid_bulk_data.header.bulkin.bSpecific=0;    /* bClockStatus = 00h Clock running
...
void RDR_to_PC_Parameters(uint8_t  errorCode)
{
  ...
  Ccid_bulk_data.header.bulkin.bMessageType = RDR_TO_PC_PARAMETERS; 
  Ccid_bulk_data.header.bulkin.bError = errorCode; 
  
  if(errorCode == SLOT_NO_ERROR)
  { 
   Ccid_bulk_data.header.bulkin.dwLength = LEN_PROTOCOL_STRUCT_T0;
  }
  else
  {
   Ccid_bulk_data.header.bulkin.dwLength = 0;
  ...
  Ccid_bulk_data.abData[0] = Protocol0_DataStructure.bmFindexDindex ;
  Ccid_bulk_data.abData[1] = Protocol0_DataStructure.bmTCCKST0;
  Ccid_bulk_data.abData[2] = Protocol0_DataStructure.bGuardTimeT0;
  Ccid_bulk_data.abData[3] = Protocol0_DataStructure.bWaitingIntegerT0;
  Ccid_bulk_data.abData[4] = Protocol0_DataStructure.bClockStop;
  
  /* bProtocolNum */
  Ccid_bulk_data.header.bulkin.bSpecific = BPROTOCOL_NUM_T0; 
...
{ 
  Ccid_bulk_data.header.bulkin.bMessageType = RDR_TO_PC_ESCAPE; 
  
  Ccid_bulk_data.header.bulkin.bSpecific=0;    /* Reserved for Future Use */
  Ccid_bulk_data.header.bulkin.bError = errorCode; 
...
void RDR_to_PC_DataRateAndClockFrequency(uint8_t  errorCode)
{
  ...
  Ccid_bulk_data.header.bulkin.bMessageType = RDR_TO_PC_DATARATEANDCLOCKFREQUENCY; 
  Ccid_bulk_data.header.bulkin.bError = errorCode; 
  Ccid_bulk_data.header.bulkin.bSpecific=0;    /* Reserved for Future Use */
...
static uint8_t CCID_CheckCommandParams (uint32_t param_type)
{
  ...
  Ccid_bulk_data.header.bulkin.bStatus = BM_ICC_PRESENT_ACTIVE | BM_COMMAND_STATUS_NO_ERROR ;
  ...
  if (parameter & CHK_PARAM_SLOT)
  {
    ...
    */
    ...
    /* Error Condition !!! */
    if (Ccid_bulk_data.header.bulkout.bSlot >= CCID_NUMBER_OF_SLOTS)
    { /* Slot requested is more than supported by Firmware */
      CCID_UpdateCommandStatus(BM_COMMAND_STATUS_FAILED,BM_ICC_NO_ICC_PRESENT);
      return SLOTERROR_BAD_SLOT;
    }
  ...
  {
    if (Ccid_bulk_data.header.bulkout.dwLength != 0)
    {
      CCID_UpdateCommandStatus(BM_COMMAND_STATUS_FAILED,BM_ICC_PRESENT_ACTIVE);
      return SLOTERROR_BAD_LENTGH;
    } 
  ...
  if (parameter & CHK_PARAM_abRFU2)
  {
    ...
    if ((Ccid_bulk_data.header.bulkout.bSpecific_1 != 0) ||
        (Ccid_bulk_data.header.bulkout.bSpecific_2 != 0))
    {
      CCID_UpdateCommandStatus(BM_COMMAND_STATUS_FAILED,BM_ICC_PRESENT_ACTIVE);
  ...
  if (parameter & CHK_PARAM_abRFU3)
  {
   /* abRFU 3 : Reserved for Future Use*/
    if ((Ccid_bulk_data.header.bulkout.bSpecific_0 != 0) || 
       (Ccid_bulk_data.header.bulkout.bSpecific_1 != 0) ||
       (Ccid_bulk_data.header.bulkout.bSpecific_2 != 0))
    {
       CCID_UpdateCommandStatus(BM_COMMAND_STATUS_FAILED,BM_ICC_PRESENT_ACTIVE);    

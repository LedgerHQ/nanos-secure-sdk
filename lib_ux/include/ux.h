...
struct ux_stack_slot_s {
    ...
    callback_int_t
        keyboard_callback;  // the keyboard/pin keyboard must know where to send the key typed
#endif                      // TARGET_BLUE
};
 ...
 * repeated, it's a single shot callback. must be reenabled (the JS way).
 */
...
#ifdef TARGET_BLUE
/**
 * Process finger events. Application's finger event handler is called only if the ux app does not
 * deny it (modal frame displayed).
 */

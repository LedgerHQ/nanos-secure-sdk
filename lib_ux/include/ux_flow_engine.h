...
struct ux_flow_step_s {
    // when NULL, cast flow_step structure as a ux_flow_step_jump_t and switch to the referenced
    // flow
    ...
    const ux_flow_step_t *const
        *validate_flow;  // for both button actions to jump onto a new flow, use init of first step
                         // of the flow to trigger a custom function upon validate. that is quite
                         // compact.
    // when NULL, error are ignored
    const ux_flow_step_t *const
        *error_flow;  // for both button actions to jump onto a new flow, use init of first step of
                      // the flow to trigger a custom function upon validate. that is quite compact.
};
 ...
 */
unsigned int ux_flow_relayout(void);  // ask for a redisplay of the current displayed step (calling
                                      // step init function again (recomputing layout if required))
...
/**
 * Macro that defines a fake flow of a single step to perform code execution upon validate/error
 * next etc
 */

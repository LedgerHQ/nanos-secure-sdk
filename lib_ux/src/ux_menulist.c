...
const ux_flow_step_t ux_menulist_conststep
    = {ux_menulist_refresh, &G_ux.menulist_params, NULL, NULL};
UX_FLOW(ux_menulist_constflow, &ux_menulist_conststep);
                             ...
                             unsigned int       selected_item_idx)
{
    ...
    // ensure the current flow step reference the G_ux.menulist_params to ensure strings displayed
    // correctly. if not, then use the forged step (and display it if top of ux stack)

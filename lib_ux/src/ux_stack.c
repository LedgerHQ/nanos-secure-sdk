...
unsigned int ux_stack_pop(void)
{
    ...
    // ask for a complete redraw (optimisation due to blink must be avoided as we're returning from
    // a modal, and within the bolos ux screen stack)
    else {
...
void ux_stack_display_elements(ux_stack_slot_t *slot)
{
...
#endif  // HAVE_BOLOS
    {
        for (i = 0; i < UX_STACK_SLOT_ARRAY_COUNT && i < slot->element_arrays_count; i++) {
            // compute elem_idx in the current array (element_index refers to the total number of
            // element to display)
        ...
        if (slot->element_index == total_element_count) {
            ...
            if (slot->displayed_callback) {
                // if screen displayed callback requested one more round, then set CONTINUE exit
                // code
...
#ifdef HAVE_SE_SCREEN
        ux_stack_display_elements(&G_ux.stack[stack_slot]);  // on balenos, no need to wait for the

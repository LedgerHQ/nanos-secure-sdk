...
void check_audited_app(void)
{
    ...
    if ((length) && (CHECK_NOT_AUDITED_TLV_VAL == data)) {
        ...
        G_ux.stack[slot].element_arrays[0].element_array_count
        ...
        // We pop the reserved slot but we do not care about the returned value (since we do not
        // need it for further displays at the moment) and reinitialize the UX and buttons.
        ux_stack_pop();

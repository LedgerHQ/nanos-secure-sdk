    ...
    switch (buffer[2]) {
        case TAG_GET_PROTOCOL_VERSION:
            ...
            ledger_protocol->chunk_length
                = MIN(sizeof(protocol_version), (sizeof(ledger_protocol->chunk) - 3));
            memcpy(&ledger_protocol->chunk[3], protocol_version, ledger_protocol->chunk_length);
        ...
        case TAG_PING:
            PRINTF("TAG_PING\n");
            ledger_protocol->chunk_length = MIN(sizeof(ledger_protocol->chunk), length);
            memcpy(ledger_protocol->chunk, buffer, ledger_protocol->chunk_length);
            break;
...
void LEDGER_PROTOCOL_tx(uint8_t *buffer, uint16_t length)
{
    ...
    U2BE_ENCODE(ledger_protocol->chunk, chunk_offset, ledger_protocol->tx_apdu_sequence_number);
    chunk_offset += 2;

    if (ledger_protocol->tx_apdu_sequence_number == 0) {
        U2BE_ENCODE(ledger_protocol->chunk, chunk_offset, ledger_protocol->tx_apdu_length);
        chunk_offset += 2;
    }
    if ((ledger_protocol->tx_apdu_length + chunk_offset)

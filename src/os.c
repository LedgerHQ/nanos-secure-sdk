...
// <tag> <length> <value>
// tag: 1 byte only
// length: 1 byte if little than 0x80, else 1 byte of length encoding (0x8Y, with Y the number of
// following bytes the length is encoded on) and then Y bytes of BE encoded total length value: no
// encoding, raw data
                             ...
                             unsigned int   maxlength)
{
    ...
    while (remlen >= 2) {
        ...
        if (tlvtag == (tag & 0xFF)) {
            ...
            if (offset > mem_len || maxlength > mem_len
            ...
            // retrieve the tlv's data content at the requested offset, and return the total data
            // length
            if (get_address) {
                *buffer = tlv + offset;

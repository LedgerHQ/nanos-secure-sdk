...
#ifdef HAVE_BOLOS
#ifdef HAVE_BLE
#if (IO_SEPROXYHAL_BUFFER_SIZE_B != 192) \
    && (defined(BOLOS_RELEASE) || !defined(DEBUG_VARIABLE_SPI_SIZE))
#error IO_SEPROXYHAL_BUFFER_SIZE_B size is invalid
#endif  // (IO_SEPROXYHAL_BUFFER_SIZE_B != 192) && (defined(BOLOS_RELEASE) ||
        // !defined(DEBUG_VARIABLE_SPI_SIZE))
#else   // !HAVE_BLE
#if (IO_SEPROXYHAL_BUFFER_SIZE_B != 128) \
    && (defined(BOLOS_RELEASE) || !defined(DEBUG_VARIABLE_SPI_SIZE))
#error IO_SEPROXYHAL_BUFFER_SIZE_B size is invalid
    ...
    for (;;) {
        if (!(G_io_app.io_flags & IO_FINISHED)) {
            G_io_app.apdu_length
                = io_exchange(CHANNEL_APDU | G_io_app.io_flags, G_io_app.apdu_length);
            // mark IO as ended

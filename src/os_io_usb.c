 ...
 *
 *  Tags:
 *  Direction:Host>Token T:0x00 V:no  Get protocol version big endian encoded. Replied with a
 * protocol-version. Channel identifier is ignored for this command. Direction:Token>Host T:0x00
 * V:yes protocol-version-4-bytes-big-endian. Channel identifier is ignored for this reply.
 *  Direction:Host>Token T:0x01 V:no  Allocate channel. Replied with a channel identifier. Channel
 * identifier is ignored for this command. Direction:Token>Host T:0x01 V:yes
 * channel-identifier-2-bytes. Channel identifier is ignored for this reply. Direction:* T:0x02 V:no
 * Ping. replied with a ping. Channel identifier is ignored for this command. NOTSUPPORTED
 * Direction:*          T:0x03 V:no  Abort. replied with an abort if accepted, else not replied.
 *  Direction:*          T:0x05 V=<sequence-idx-U16><seq==0?totallength:NONE><apducontent> APDU
                                               ...
                                               unsigned short l)
{
    ...
    switch (G_io_usb_ep_buffer[2]) {
        case 0x05:
            ...
            if ((unsigned int) U2BE(G_io_usb_ep_buffer, 3)
                != (unsigned int) G_io_usb_hid_sequence_number) {
                // ignore packet
                goto apdu_reset;
            }
            // cid, tag, seq
            ...
            if (G_io_usb_hid_sequence_number == 0) {
                ...
                G_io_usb_hid_total_length
                    = U2BE(G_io_usb_ep_buffer,
                           5);  //(G_io_usb_ep_buffer[5]<<8)+(G_io_usb_ep_buffer[6]&0xFF);
                // check for invalid length encoding (more data in chunk that announced in the total
                // apdu)
                ...
                // copy data
                memmove(G_io_usb_hid_current_buffer, G_io_usb_ep_buffer + 7, l);
            }
            else {
                // check for invalid length encoding (more data in chunk that announced in the total
                // apdu)
                ...
                // append content
                memmove(G_io_usb_hid_current_buffer, G_io_usb_ep_buffer + 5, l);
            }
            // factorize (f)
...
void io_usb_hid_sent(io_send_t sndfct)
{
    ...
    if (G_io_usb_hid_remaining_length && G_io_usb_hid_current_buffer) {
        ...
        if (G_io_usb_hid_sequence_number == 0) {
            l                     = ((G_io_usb_hid_remaining_length > IO_HID_EP_LENGTH - 7)
                                         ? IO_HID_EP_LENGTH - 7
                                         : G_io_usb_hid_remaining_length);
            G_io_usb_ep_buffer[5] = G_io_usb_hid_remaining_length >> 8;
            G_io_usb_ep_buffer[6] = G_io_usb_hid_remaining_length;
            memmove(G_io_usb_ep_buffer + 7, G_io_usb_hid_current_buffer, l);
            G_io_usb_hid_current_buffer += l;
            G_io_usb_hid_remaining_length -= l;
        }
        else {
            l = ((G_io_usb_hid_remaining_length > IO_HID_EP_LENGTH - 5)
                     ? IO_HID_EP_LENGTH - 5
                     : G_io_usb_hid_remaining_length);

...
void os_perso_set_onboarding_status(unsigned int kind,
                                    unsigned int count,
...
unsigned int os_endorsement_get_public_key_certificate(unsigned char index, unsigned char *buffer)
{
    ...
    parameters[0] = (unsigned int) index;
    parameters[1] = (unsigned int) buffer;
    return (unsigned int) SVC_Call(SYSCALL_os_endorsement_get_public_key_certificate_ID_IN,
                                   parameters);
}
...
void os_aem_set_response(unsigned char *aem_response,

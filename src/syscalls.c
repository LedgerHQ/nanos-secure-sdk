...
void os_perso_derive_eip2333 ( cx_curve_t curve, unsigned int * path, unsigned int pathLength, unsigned char * privateKey ) 
{
  unsigned int parameters [2+4];
  parameters[0] = (unsigned int)curve;
  parameters[1] = (unsigned int)path;
  ...
  if (((volatile unsigned int*)parameters)[0] != SYSCALL_os_perso_derive_eip2333_ID_OUT) { // force reread, updated by the svc
    THROW(EXCEPTION_SECURITY);
  }
}
...
void os_lib_call ( unsigned int * call_parameters ) 
{
  ...
  // We don't force the reread here since the OS might have changed the value itself.
}
...
unsigned int os_bootloader_version ( unsigned char * version, unsigned int maxlength ) 
{
  ...
  if (((volatile unsigned int*)parameters)[0] != SYSCALL_os_bootloader_version_ID_OUT) { // force reread, updated by the svc
    THROW(EXCEPTION_SECURITY);
  }
  return (unsigned int)(((volatile unsigned int*)parameters)[1]);
}
